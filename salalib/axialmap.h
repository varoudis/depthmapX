...
public:
   ...
   //void initAttributes();
   bool integrate(Communicator *comm = NULL, const std::set<int> &radius = std::set<int>(), bool choice = false, bool local = false, bool fulloutput = false, int weighting_col = -1, bool simple_version = true);
   bool stepdepth(Communicator *comm = NULL);
   bool analyseAngular(Communicator *comm, const std::set<double>& radius);
   // extra parameters for selection_only and interactive are for parallel process extensions
   int analyseTulip(Communicator *comm, int tulip_bins, bool choice, int radius_type, const std::set<double>& radius, int weighting_col, int weighting_col2 = -1, int routeweight_col = -1, bool selection_only = false, bool interactive = true);
