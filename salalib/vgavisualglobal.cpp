#include "salalib/vgavisualglobal.h"
...
bool VGAVisualGlobal::run(Communicator *comm, Options &options, PointMap &map, bool simple_version) {
    ...
    if (!simple_version) {
        if (options.local) {
            cluster_col = attributes.insertColumn("Visual Clustering Coefficient");
            control_col = attributes.insertColumn("Visual Control");
            controllability_col = attributes.insertColumn("Visual Controllability");
        }
    }
                ...
                if (options.global) {
                    ...
                    for (int ii = 0; ii < map.getCols(); ii++) {
                        for (int jj = 0; jj < map.getRows(); jj++) {
                            map.m_points[size_t(ii * map.getRows() + jj)].m_misc = 0;
                            map.m_points[size_t(ii * map.getRows() + jj)].m_extent = PixelRef(ii, jj);
                        }
                    }
                        ...
                        for (size_t n = search_tree[level].size() - 1; n != paftl::npos; n--) {
                            ...
                            if (p.filled() && p.m_misc != ~0) {
                                ...
                                if ((int)options.radius == -1 ||
                                    ...
                                    p.m_misc = ~0;
                                    if (!p.m_merge.empty()) {
                                        Point &p2 = map.getPoint(p.m_merge);
                                        if (p2.m_misc != ~0) {
                                            p2.m_node->extractUnseen(search_tree[level + 1], this,
