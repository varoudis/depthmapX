...
void CGridDialog::OnOK()
{
	...
	if (m_spacing > truemax || m_spacing < truemin) {
        ...
        absminstr.sprintf(formatabsmin.toStdString().data(), truemin/10);
        minstr.sprintf(formatmin.toStdString().data(), truemin);
        maxstr.sprintf(formatmax.toStdString().data(), truemax);
